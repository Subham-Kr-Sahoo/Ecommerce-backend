...
module.exports.controller = (app,io,socket_list)=>{
    ...
    const msg_invalid_item = "Invalid item"
    const msg_already_register = "This email is already registered"
    const msg_favourite_added = "Added to the favourite"
    const msg_favourite_removed = "Removed from the favourite"
    const msg_cart_added = "Added to cart successfully"
    const msg_cart_updated = "Item updated quantity to cart successfully"
    const msg_cart_remove = "Removed from cart successfully"
    ...
    app.post('/api/app/login', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["email", "password", "device_token"], () => {
            ...
            db.query("UPDATE `user_details` SET `auth_token`= ?,`device_token`=?,`modify_date`= NOW() WHERE `user_type` = ? AND `email` = ? AND `password` = ? AND `status` = ?", [auth_token, reqObj.device_token, "1", reqObj.email, reqObj.password, "1"], (err, result) => {
                ...
                if (result.affectedRows > 0) {
                    db.query('SELECT `user_id`, `username`, `name`, `email`, `mobile`, `mobile_code`,`auth_token`, `status`, `created_date` FROM `user_details` WHERE `email` = ? AND `password` = ? AND `status` = "1" ', [reqObj.email, reqObj.password], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
    ...
    app.post('/api/app/sign_up',(req,res)=>{
        ...
        helper.CheckParameterValid(res,reqObj,["username","email","password","device_token"],()=>{
            db.query('SELECT `user_id`, `status` FROM `user_details` WHERE `email` = ? ',[ reqObj.email],(err,result) => {
                if(err){
                    helper.ThrowHtmlError(err,res);
                    return
                }
                if(result.length > 0){
                    ...
                    var auth_token = helper.createRequestToken();
                    db.query("INSERT INTO `user_details`(`username`, `email`, `password`, `auth_token`, `device_token`,`created_date`,`modify_date`) VALUES(?,?,?,?,?,NOW(),NOW())",[reqObj.username,reqObj.email,reqObj.password,auth_token,reqObj.device_token],(err,result)=>{
                        if(err){
                            helper.ThrowHtmlError(err,res);
                            return
                        }
                        if(result){
                            db.query('SELECT `user_id`, `username`, `name`, `email`, `mobile`, `mobile_code`, `password`, `auth_token`,`status`, `created_date` FROM `user_details` WHERE `user_id` = ? AND `status` = "1"',[result.insertId],(err,result) => {
                                if(err){
                                    helper.ThrowHtmlError(err,res);
                                    return
                                }
    ...
    app.post('/api/app/explore_category_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            ...
            db.query("SELECT `cat_id`, `cat_name`, (CASE WHEN `image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `image` ) ELSE '' END) AS `image` , `color` FROM `category_details` WHERE `status` = 1 ", [], (err, result) => {
                if (err) {
                    helper.ThrowHtmlError(err, res);
                    return
                }
    ...
    app.post('/api/app/cart_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            getUserCart(res, userObj.user_id, (result, total) => {
                ...
                db.query(
                'SELECT `promo_code_id`, `min_order_amount`, `max_discount_amount`, `offer_price` FROM `promo_code_details` WHERE  `start_date` <= NOW() AND `end_date` >= NOW()  AND `status` = 1  AND `promo_code_id` = ? ;'
            ...
            helper.CheckParameterValid(res, reqObj, ["address_id"], () => {
                db.query("UPDATE `address_detail` SET `is_default` = (CASE WHEN `address_id` = ? THEN 1 ELSE 0 END) , `modify_date`= NOW() WHERE `user_id` = ? AND `status` = 1 ", [reqObj.address_id, userObj.user_id], (err, result) => {
                    ...
                    if (result.affectedRows > 0) {
                        res.json({
                            "status": "1",
                            "message": msg_success + "ly marked your address as default"
                        })
                    } else {
                        res.json({
                            "status": "0",
    ...
    app.post('/api/app/update_profile', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["username", "name", "mobile", "mobile_code"], () => {
                db.query("UPDATE `user_details` SET `username`=?,`name`=?,`mobile`=?,`mobile_code`=?,`modify_date`=NOW() WHERE `user_id` = ? AND `status` = 1", [reqObj.username, reqObj.name, reqObj.mobile, reqObj.mobile_code, userObj.user_id], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res)
                        return
                    }
                    if (result.affectedRows > 0) {
                        db.query('SELECT `user_id`, `username`, `name`, `email`, `mobile`, `mobile_code`, `password`, `auth_token`, `status`, `created_date` FROM `user_details` WHERE `user_id` = ? AND `status` = "1" ', [userObj.user_id], (err, result) => {
    ...
    app.post('/api/app/change_password', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["current_password", "new_password"], () => {
                db.query("UPDATE `user_details` SET `password`=?, `modify_date`=NOW() WHERE `user_id` = ? AND `password` = ?", [reqObj.new_password, userObj.user_id, reqObj.current_password], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res)
                        return
                    }
    ...
    app.post('/api/app/forgot_password_request', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["email"], () => {
            db.query("SELECT `user_id` FROM `user_details` WHERE `email` = ? ", [reqObj.email], (err, result) => {
                if (err) {
                    helper.ThrowHtmlError(err, res)
                    return
                }
...
function getProductDetail(res, prod_id, user_id) {
    db.query("SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`brand_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` , IFNULL( `bd`.`brand_name`, '' ) AS `brand_name` , `td`.`type_name`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active`, AVG( CASE WHEN `rd`.`rate` IS NOT NULL THEN `rd`.`rate` ELSE 0 END ) AS `avg_rating` FROM `product_detail` AS  `pd` " +
        ...
        "INNER JOIN `user_details` AS `ud` ON `ud`.`user_id` = `rd`.`user_id` AND `rd`.`prod_id` = ? ORDER BY `rd`.`review_id` DESC LIMIT 5", [user_id, "1", prod_id, prod_id, "1", prod_id, "1", prod_id], (err, result) => {
        if (err) {
            helper.ThrowHtmlError(err, res);
            return;
        }
        if (result[0].length > 0) {
            ...
            res.json({"status": "1", "payload": result[0][0]});
        } else {
